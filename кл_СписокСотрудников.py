...
class СписокСотрудников(ttk.Frame):
    ...
    def _создать_фреймы(self):
        ...
        # Создаем левый фрейм для меню, прилипающий к левому краю
        self.Меню_списка_сотрудников = ttk.Frame(основной_контейнер, width=200)
        self.Меню_списка_сотрудников.pack(side='left', fill='y')
        # Фиксируем ширину левого фрейма
        self.Меню_списка_сотрудников.pack_propagate(False)
        
        # Создаем правый фрейм для таблицы
        self.Таблица_сотрудников = ttk.Frame(основной_контейнер)
        self.Таблица_сотрудников.pack(side='right', fill='both', expand=True)
        
        # Создаем пустой фрейм для заполнения пространства
        пустой_фрейм = ttk.Frame(self.Меню_списка_сотрудников)
        пустой_фрейм.pack(fill='both', expand=True)
        
        # Создаем группу "Добавить" для кнопок в нижней части левого фрейма
        группа_добавить = ttk.LabelFrame(self.Меню_списка_сотрудников, text="Добавить", padding=10)
        группа_добавить.pack(fill='x', pady=5, padx=5, side='bottom')
    ...
    def _проверить_наличие_записей(self):
        ...
        try:
            ...
            # Обновленный код для корректной обработки результата
            количество = результат if isinstance(результат, int) else (результат[0]['count'] if результат else 0)
    ...
    def проверить_наличие_записей(self, тип_сотрудника):
        """Проверяет наличие записей в таблице"""
        try:
            таблица = 'курсанты' if тип_сотрудника == 'курсант' else 'офицеры'
            запрос = f"SELECT COUNT(*) as count FROM {таблица}"
            результат = выполнить_запрос(запрос)
            # результат теперь является числом, возвращаем его напрямую
            return результат > 0
        except Exception as e:
            logger.error(f"Ошибка при проверке наличия записей: {e}")
            return False
