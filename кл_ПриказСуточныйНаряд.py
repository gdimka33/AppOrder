...
class ПриказСуточныйНаряд(ttk.Frame):
    ...
    def _создать_элементы_формы(self):
        ...
        # Создаем контейнер, который будет содержать левую и правую части
        контейнер = ttk.Frame(self.форма_приказа, width=600)
        контейнер.pack(side='left', fill='y', anchor='w', expand=True)
        ...
        основные_данные = ttk.LabelFrame(левая_часть, text="Основные данные", padding=10)
        ...
        ответственные = ttk.LabelFrame(левая_часть, text="Ответственные лица", padding=10)
        ...
        # Лица для согласования (через запятую)
        ...
        # Добавляем календарь выбора дат под основными данными в левой части
        from кл_ВыборДатыСоставаНаряда import ВыборДатыСоставаНаряда
        self.выбор_дат = ВыборДатыСоставаНаряда(левая_часть, callback=self._обновить_состав_наряда)
        self.выбор_дат.pack(fill='both', expand=True, pady=5)
        
        # Создаем правую часть для состава наряда
        self.правая_часть = ttk.Frame(контейнер)
        # Цвет фона светло красный
        self.правая_часть.configure(style="LightRed.TFrame")
        # Устанавливаем ширину правой части
        self.правая_часть.configure(width=350)
        # Выравнивание по левому краю, растянуть по высоте
        self.правая_часть.pack(side='left', fill="y", anchor='w', expand=True)
        ...
        # Создаем фрейм для состава наряда
        self.фрейм_состава_наряда = ttk.Frame(self.правая_часть)
        self.фрейм_состава_наряда.pack(fill='both', expand=True)

    def _обновить_состав_наряда(self, дата):
        """Обновляет отображение состава наряда для выбранной даты"""
        # Инициализируем блокнот с вкладками, если его еще нет
        if not hasattr(self, 'notebook'):
            # Удаляем старый фрейм состава наряда
            for виджет in self.фрейм_состава_наряда.winfo_children():
                виджет.destroy()
            
            # Создаем блокнот с вкладками
            self.notebook = ttk.Notebook(self.фрейм_состава_наряда)
            self.notebook.pack(fill='both', expand=True, padx=5, pady=5)
            
            # Словарь для хранения фреймов по датам
            self.фреймы_нарядов = {}
        ...
        if дата in self.фреймы_нарядов:
            ...
            # Добавляем фрейм в блокнот как новую вкладку
