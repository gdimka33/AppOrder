...
class ПриказСуточныйНаряд(ttk.Frame):
    ...
    def _обновить_состав_наряда(self, дата):
        ...
        # Очищаем текущий состав наряда
        for виджет in self.фрейм_состава_наряда.winfo_children():
            виджет.destroy()
            
        # Отображаем метку с выбранной датой
        ttk.Label(
            self.фрейм_состава_наряда,
            text=f"Состав наряда на {дата.strftime('%d.%m.%Y')}",
            font=('Arial', 10, 'bold')
        ).pack(pady=5)
        
        # Создаем экземпляр класса СоставНаряда
        from кл_СоставНаряда import СоставНаряда
        состав_наряда = СоставНаряда(self.фрейм_состава_наряда)
