...
class ДобавлениеСотрудника(ttk.Frame):
    ...
    def _создать_содержимое(self):
        ...
        # Создаем фрейм для полей ввода с возможностью прокрутки
        канва = tk.Canvas(фрейм_содержимого)
        прокрутка = ttk.Scrollbar(фрейм_содержимого, orient="vertical", command=канва.yview)
        фрейм_полей = ttk.Frame(канва)
        
        # Настраиваем прокрутку
        канва.configure(yscrollcommand=прокрутка.set)
        канва.pack(side="left", fill="both", expand=True)
        прокрутка.pack(side="right", fill="y")
        
        # Создаем окно в канве для размещения фрейма полей
        канва.create_window((0, 0), window=фрейм_полей, anchor="nw")
        фрейм_полей.bind("<Configure>", lambda e: канва.configure(scrollregion=канва.bbox("all")))
        ...
        for i, поле in enumerate(self.структура_полей):
            ...
            # В методе _создать_содержимое() добавьте обработку типа REFERENCE:
            # Найдите блок, где создаются виджеты в зависимости от типа поля, и добавьте:
        ...
        фрейм_кнопок.pack(fill="x", pady=20)
        
        # Кнопки Сохранить и Отмена
        ttk.Button(фрейм_кнопок, text="Сохранить", command=self._сохранить_сотрудника).pack(side="left", padx=5)
        ttk.Button(фрейм_кнопок, text="Отмена", command=self._отмена).pack(side="left", padx=5)
