...
from виджеты import ВиджетЗвания
...
class ДобавлениеСотрудника(ttk.Frame):
        ...
        try:
            ...
            for поле in self.структура_полей:
                ...
                if поле['тип'] == 'REFERENCE':
                    if имя_поля == 'звание':
                        # Для звания используем метод get_id() нашего виджета
                        виджет = self.поля_ввода[имя_поля]
    ...
    def _получить_значение_поля(self, имя_поля):
        ...
        # Для виджета звания
        if имя_поля == 'звание' and isinstance(виджет, ВиджетЗвания):
            return виджет.get()
