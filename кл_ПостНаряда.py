...
class ПостНаряда(ttk.Frame):
    def __init__(
        ...
        **kwargs
        ...
        # Формируем уникальное имя объекта из даты и названия поста
        self.имя_объекта = self._создать_имя_объекта()
    ...
    def _создать_имя_объекта(self) -> str:
        """
        Создает уникальное имя объекта на основе даты наряда и наименования поста.
        
        Returns:
            str: Имя объекта в формате 'дд_мм_гггг_наименование_поста'
        """
        # Форматируем дату в строку вида дд_мм_гггг
        дата_строкой = self.дата_наряда.strftime('%d_%m_%Y')
        
        # Получаем наименование поста из загруженных данных
        наименование = self.данные_поста['наименование']
        
        # Заменяем пробелы и специальные символы на подчеркивание
        наименование = наименование.replace(' ', '_').replace('-', '_')
        
        # Формируем итоговое имя объекта
        return f"{дата_строкой}_{наименование}"

    @property
    def имя(self) -> str:
        """
        Свойство для получения имени объекта.
        
        Returns:
            str: Уникальное имя объекта
        """
        return self.имя_объекта
