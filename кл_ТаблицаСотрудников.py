...
class ТаблицаСотрудников(ttk.Frame):
    ...
    def загрузить_данные(self):
        ...
        try:
            ...
            # Теперь результат_count это число, а не словарь
            self.всего_записей = результат_count
            logger.debug(f"Общее количество записей в таблице '{таблица}': {self.всего_записей}")
            ...
            if not результат or not isinstance(результат, list):
                logger.error(f"Ошибка при выполнении основного запроса: {результат}")
                return
            
            # Проверяем структуру результата
            if not all(isinstance(row, dict) for row in результат):
                logger.error(f"Некорректная структура результата запроса: {результат}")
                return
            
            logger.debug(f"Результат основного запроса: {результат}")
            ...
            for i, row in enumerate(результат):
                # Формируем ФИО
                ...
                if self.тип_сотрудника == 'курсант':
                    ...
                    values.extend([
                        row['звание'].capitalize(),
                        row['должность'].capitalize(),
                        row['подразделение'].capitalize() if row['подразделение'] else '',
                        row['сод'],
                        row['псод']
                    ])
                
                тег = 'четная_строка' if i % 2 == 0 else ''
                self.таблица.insert('', 'end', values=values, tags=(тег,))
...
def загрузить_данные(тип_сотрудника):
    """
    Загружает данные сотрудников из базы данных
    """
    try:
        logger.info(f"Загрузка данных для типа сотрудника: {тип_сотрудника}")
        
        if тип_сотрудника == 'офицер':
            sql = """SELECT офицеры.id, фамилия, имя, отчество, 
                            звания.наименование AS звание, 
                            должности.наименование AS должность, 
                            подразделения.наименование AS подразделение
                     FROM офицеры
                     LEFT JOIN звания ON офицеры.звание_id = звания.id
                     LEFT JOIN должности ON офицеры.должность_id = должности.id
                     LEFT JOIN подразделения ON офицеры.подразделение_id = подразделения.id"""
        elif тип_сотрудника == 'курсант':
            sql = """SELECT курсанты.id, фамилия, имя, отчество, 
                            звания.наименование AS звание, 
                            должности.наименование AS должность, 
                            подразделения.наименование AS подразделение, 
                            год_набора
                     FROM курсанты
                     LEFT JOIN звания ON курсанты.звание_id = звания.id
                     LEFT JOIN должности ON курсанты.должность_id = должности.id
                     LEFT JOIN подразделения ON курсанты.подразделение_id = подразделения.id"""
        else:
            raise ValueError(f"Неизвестный тип сотрудника: {тип_сотрудника}")

        данные = выполнить_запрос(sql)
        if not данные:
            logger.warning(f"Данные для типа сотрудника '{тип_сотрудника}' отсутствуют")
            return []

        logger.info(f"Загружено {len(данные)} записей для типа сотрудника '{тип_сотрудника}'")
        return данные
