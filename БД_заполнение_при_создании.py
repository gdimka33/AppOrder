...
TABLES = {
    ...
    'посты': '''
        CREATE TABLE IF NOT EXISTS посты (
            ...
            дежурный_кол INTEGER NOT NULL DEFAULT 1,
...
def _заполнить_справочник_постов():
    ...
    посты = [
        # наименование, дежурный, дежурный_кол, дежурный_офицер, дневальный, дневальный_кол
        ('Начальник служебного наряда', 1, 1, 1, 0, 0),
        ('Помощник начальника служебного наряда', 1, 1, 1, 0, 0),
        ('КПП-1', 1, 1, 0, 1, 1),
        ('КПП-2', 1, 1, 0, 0, 0),
        ('КПП-3', 1, 1, 0, 1, 3),
        ('Общежитие', 1, 1, 0, 1, 1),
        ('Учебные корпуса', 1, 1, 0, 1, 5),
        ('1-ый этаж общежития', 1, 1, 0, 1, 2),
        ('2-ый этаж общежития', 1, 1, 0, 1, 2),
        ('3-ый этаж общежития', 1, 1, 0, 1, 2),
        ('4-ый этаж общежития', 1, 1, 0, 1, 2),
        ('5-ый этаж общежития', 1, 1, 0, 1, 2),
        ('Спортивный комплекс', 1, 1, 0, 1, 1),
        ('Столовая', 1, 1, 0, 1, 11),
        ('Изолятор', 1, 1, 0, 1, 1),
        ('Дежурный водитель', 1, 1, 1, 0, 0)
    ]
    ...
    try:
        ...
        # Заполняем таблицу данными
        for наименование, дежурный, дежурный_кол, дежурный_офицер, дневальный, дневальный_кол in посты:
            выполнить_запрос(
                "INSERT INTO посты (наименование, дежурный, дежурный_кол, дежурный_офицер, дневальный, дневальный_кол) VALUES (?, ?, ?, ?, ?, ?)",
                (наименование, дежурный, дежурный_кол, дежурный_офицер, дневальный, дневальный_кол)
            )
