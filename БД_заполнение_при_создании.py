...
def проверка_создание_таблиц():
    ...
    try:
        ...
        справочные_таблицы = {k: v for k, v in TABLES.items() if k in ['звания', 'должности', 'подразделения', 'типы_нарядов']}
        ...
        основные_таблицы = {k: v for k, v in TABLES.items() if k not in ['звания', 'должности', 'подразделения', 'типы_нарядов']}
...
def _заполнить_справочник_подразделений():
    ...
    подразделения = [
        ...
        ('Отделение воспитательной и социальной работы с личным составом', 'ОВСРЛС'),
