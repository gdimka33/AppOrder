...
def _заполнить_справочник_подразделений():
    ...
    try:
        ...
        ооо_юф_id = _добавить_подразделение('Отделение очного обучения ЮФ', 'Отделение', 'ООО ЮФ')
        озо_юф_id = _добавить_подразделение('Отделение заочного обучения ЮФ', 'Отделение', 'ОЗО ЮФ')
        ...
        кафедры_юр = [
            ...
            ('Кафедра гражданско-правовых дисциплин', 'ГрПД'),
            ('Кафедра оперативно-розыскной деятельности', 'ОРД'),
            ('Кафедра организации режима и надзора', 'ОРН'),
            ('Кафедра уголовного права и криминологии', 'УПК'),
            ('Кафедра уголовно-процессуального права и криминалистики', 'УППК'),
            ('Кафедра уголовно-исполнительного права', 'УИП'),
            ('Кафедра управления и административно-правовых дисциплин', 'УАПД'),
            ('Кафедра организации деятельности оперативных аппаратов УИС и специальных мероприятий', 'ОДОА')
        ]
        ...
        горпк_id = _добавить_подразделение('Группа организации работы по противодействию коррупции', 'Группа', 'ГОРПК')
...
def _заполнить_справочник_постов():
    ...
    посты = [
        ...
        ('Помощник начальника служебного наряда', 1, 1, 1, 1, 0, 0),  # Можно выбрать офицера или курсанта
        ('Дежурный водитель', 1, 1, 1, 0, 0, 0),
        ('КПП-1', 1, 1, 0, 1, 1, 1),  # Только курсант
        ('КПП-2', 1, 1, 0, 1, 0, 0),  # Только курсант
        ('КПП-3', 1, 1, 0, 1, 1, 3),  # Только курсант
        ('Патруль', 1, 1, 0, 1, 1, 3),  # Только курсант
        ('Общежитие', 1, 1, 0, 1, 1, 1),  # Только курсант
        ('Учебные корпуса', 1, 1, 0, 1, 1, 5),  # Только курсант
        ('1-ый этаж общежития', 1, 1, 0, 1, 1, 2),  # Только курсант
        ('2-ый этаж общежития', 1, 1, 0, 1, 1, 2),  # Только курсант
        ('3-ый этаж общежития', 1, 1, 0, 1, 1, 2),  # Только курсант
        ('4-ый этаж общежития', 1, 1, 0, 1, 1, 2),  # Только курсант
        ('5-ый этаж общежития', 1, 1, 0, 1, 1, 2),  # Только курсант
        ('Спортивный комплекс', 1, 1, 0, 1, 1, 1),  # Только курсант
        ('Столовая', 1, 1, 0, 1, 1, 11),  # Только курсант
        ('Изолятор', 1, 1, 0, 1, 1, 1)  # Только курсант
    ]
    
    try:
        # Проверяем, есть ли уже данные в таблице
        результат = выполнить_запрос("SELECT COUNT(*) FROM посты")
        if результат and результат[0][0] > 0:
            logger.info("Справочник постов уже заполнен")
            return
            
        # Заполняем таблицу данными
        for наименование, дежурный, дежурный_кол, дежурный_офицер, дежурный_курсант, дневальный, дневальный_кол in посты:
            выполнить_запрос(
                """INSERT INTO посты (
...
def _заполнить_справочник_типов_нарядов():
    ...
    try:
        # Проверяем, есть ли уже данные в таблице
        результат = выполнить_запрос("SELECT COUNT(*) FROM типы_нарядов")
        if результат and результат[0][0] > 0:
