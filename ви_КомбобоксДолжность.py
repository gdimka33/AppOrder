...
class ВиджетДолжности(ttk.Combobox):
    ...
    def _получить_должности_из_бд(self):
        ...
        try:
            # Изменяем условие для работы с JSON полем категории
            if self.тип_сотрудника == "офицер":
                условие = "json_extract(категории, '$') LIKE '%\"офицер\"%' OR json_extract(категории, '$') LIKE '%\"общее\"%'"
            elif self.тип_сотрудника == "курсант":
                условие = "json_extract(категории, '$') LIKE '%\"курсант\"%' OR json_extract(категории, '$') LIKE '%\"общее\"%'"
            else:
